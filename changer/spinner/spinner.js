...
$.widget('ui.spinner', {
	...
	_createButtons: function(input) {
		...
		var self = this,
			...
			wrapper = self.wrapper = input.wrap('<span class="spinner-wrpr" />').css({ width: (self.oWidth = (box ? input.width() : input.outerWidth())) - buttonWidth, 
												 marginRight: '19px', marginLeft: '19px', textAlign: 'center', 'float': 'none', marginTop: 0 })
		...
		wrapper.append(btnContainer.css({ height: height, left: 0,
			// use offset calculation to fix vertical position in Firefox
			top: 0 }));
		
		buttons = self.buttons = btnContainer.find('.ui-spinner-button');
		buttons.css({ width: '21px', height: height - (box ? buttons.outerHeight() - buttons.height() : 0) });
		buttons.eq(0).css({ right: '3px' });
		buttons.eq(1).css({ left: '2px' });
		upButton = buttons[0];
